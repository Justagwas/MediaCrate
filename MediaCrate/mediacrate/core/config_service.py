...
DEFAULT_FILENAME_TEMPLATE = "%(title).150B [%(id)s].%(ext)s"
...
def default_config() -> AppConfig:
    return AppConfig(
        ...
        download_location=str(default_download_dir()),
        batch_enabled=False,
        batch_concurrency=4,
...
def config_path() -> Path:
    return runtime_storage_dir() / CONFIG_FILENAME


def _legacy_config_candidates() -> list[Path]:
    candidates: list[Path] = []
    seen: set[Path] = set()
    bases = [runtime_storage_dir(), app_dir(), appdata_dir()]
    for base in bases:
